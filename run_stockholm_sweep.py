                    ...
                    if is_labeled.any():
                        ...
                        if not is_labeled.all() and is_sthlm.any():
                            if sthlm_buildings.INCLUDE:
                                ...
                                if select_t1.any():
                                    sem_stream1_t1_loss += sup_criterion(logits_stream1_sem_t1[select_t1],
                                                                         y_sem_t1[select_t1])
                                    sem_stream2_t1_loss += sup_criterion(logits_stream2_sem_t1[select_t1],
                                                                         y_sem_t1[select_t1])
                                    sem_fusion_t1_loss += sup_criterion(logits_fusion_sem_t1[select_t1],
                                                                        y_sem_t1[select_t1])
                                if select_t2.any():
                                    sem_stream1_t2_loss += sup_criterion(logits_stream1_sem_t2[select_t2],
                                                                         y_sem_t2[select_t2])
                                    sem_stream2_t2_loss += sup_criterion(logits_stream2_sem_t2[select_t2],
                                                                         y_sem_t2[select_t2])
                                    sem_fusion_t2_loss += sup_criterion(logits_fusion_sem_t2[select_t2],
                                                                        y_sem_t2[select_t2])

                        sem_loss = (
                                               sem_stream1_t1_loss + sem_stream1_t2_loss + sem_stream2_t1_loss + sem_stream2_t2_loss +
