        ...
        with wandb.init(config=sweep_cfg):
            ...
            for epoch in range(1, epochs + 1):
                ...
                for i, batch in enumerate(dataloader):
                    ...
                    change_loss = criterion(logits_change, y_change)

                    # semantics
                    y_sem_t1 = batch['y_sem_t1'].to(device)
                    sem_stream1_t1_loss = criterion(logits_stream1_sem_t1, y_sem_t1)
                    sem_stream2_t1_loss = criterion(logits_stream2_sem_t1, y_sem_t1)
                    sem_fusion_t1_loss = criterion(logits_fusion_sem_t1, y_sem_t1)

                    y_sem_t2 = batch['y_sem_t2'].to(device)
                    sem_stream1_t2_loss = criterion(logits_stream1_sem_t2, y_sem_t2)
                    sem_stream2_t2_loss = criterion(logits_stream2_sem_t2, y_sem_t2)
                    sem_fusion_t2_loss = criterion(logits_fusion_sem_t2, y_sem_t2)
